...
def plot_route(G, pub_crawl, address_point, selected_pubs):
    ...
    # create a map object
    m = folium.Map(
        location=[address_point[0], address_point[1]],
        zoom_start=16,
        tiles="OpenStreetMap",
    )
    ...
    for pub, row in selected_pubs.iterrows():
        tooltip = "Click me!"
        iframe = folium.IFrame("Bar Name: " + str(pub))
    ...
    # <FontAwesomeIcon icon="fa-light fa-beer-mug" />

    folium.Marker(
        [address_point[0], address_point[1]],
        popup="Start",
        tooltip=tooltip,
        icon=folium.Icon(color="green", prefix="fa", icon="home"),
    ).add_to(m)

    # add custom leaflet layers
    ...
    for edge in G.edges:
        # if edge was "simplified", it has a "geometry" attribute
    ...
    my_polyline = folium.PolyLine(locations=mloc, weight=1, color="black").add_to(mnw)
...
def pubcrawl_route(start_address, no_bars, network_type="walk"):
    ...
    print("total travel time is ", round(total_time / 60), " minutes")
